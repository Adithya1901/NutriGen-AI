...
function MealPlanPage() {
  ...
  const parseMealDetails = (planText) => {
    if (!planText) {
      return { meal_name: "Indian Home Meal", description: "", components: [], estimated_cost: 0, calories: 0, protein_g: 0, carbohydrates_g: 0, fat_g: 0, image_url: "", youtube_url: "" };
    }
    try {
      const parsed = typeof planText === 'string' ? JSON.parse(planText) : planText;
      if (parsed && typeof parsed === 'object') {
        const mealName = parsed.name || parsed.meal_name || "Indian Meal";
        ...
        const firstCompImg = comps[0] ? (comps[0].image_url || "") : "";
        const firstCompYt = comps[0] ? (comps[0].youtube_search_url || comps[0].youtube_url || "") : "";
        ...
        return {
          ...
          steps: parsed.steps || [],
          prep_time: parsed.prep_time || "10 mins",
          cook_time: parsed.cook_time || "15 mins",
          servings: parsed.servings || 4,
          estimated_cost: parsed.estimated_price_inr || parsed.estimated_cost || 0,
          estimated_price_inr: parsed.estimated_price_inr || parsed.estimated_cost || 0,
          calories: parsed.calories || 0,
          protein_g: parsed.protein_g || 0,
          carbohydrates_g: parsed.carbs_g || parsed.carbohydrates_g || 0,
          carbs_g: parsed.carbs_g || parsed.carbohydrates_g || 0,
          fat_g: parsed.fat_g || 0,
          image_url: parsed.image_url || firstCompImg || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=800&q=80",
          youtube_url: parsed.youtube_url || firstCompYt || `https://www.youtube.com/results?search_query=${encodeURIComponent(mealName + " recipe")}`
        };
      }
    } catch (e) {
    ...
    return {
      ...
      youtube_url: `https://www.youtube.com/results?search_query=${encodeURIComponent(firstLine + " recipe")}`
    };
  };
      ...
      {plans.length > 0 && !loading && (
        <div className="anim-fade-in">
          {Object.entries(groupedPlans).map(([date, datePlans]) => {
            const dailyTotalCost = datePlans.reduce((sum, p) => {
              const details = parseMealDetails(p.plan_text);
              return sum + (details.estimated_cost || 0);
            }, 0);

            return (
              <div key={date} style={{ marginBottom: "30px" }}>
                <h2 style={{ color: "#fff", borderBottom: "2px solid #38bdf8", paddingBottom: "10px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "10px" }}>
                  <span>Target Date: {date} <span style={{ fontSize: "0.75em", color: "#34d399", marginLeft: "10px" }}>Daily Total: ₹{dailyTotalCost.toFixed(2)}</span></span>
                  <button 
                    ...
                    return (
                      <div key={idx} className="card daily-plan-card" style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                        <div>
                          ...
                          {/* Food Image */}
                          {mealDetails.image_url && (
                            <div style={{ marginBottom: "12px", borderRadius: "10px", overflow: "hidden", height: "140px" }}>
                              <img 
